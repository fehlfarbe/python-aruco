...
rm -rf 27-sdist && virtualenv 27-sdist && 27-sdist/bin/pip install dist/aruco-3.1.2.0-cp27-cp27mu-linux_x86_64.whl && 27-sdist/bin/python example/example.py

# create python3 env and run example
rm -rf 36-sdist && virtualenv -p python3 36-sdist && 36-sdist/bin/pip install dist/aruco-3.1.2.0-cp36-cp36m-linux_x86_64.whl && 36-sdist/bin/python example/example.py
